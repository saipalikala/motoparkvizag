...
- **AccountPage was never verified in a browser** — auth-gated, and a forged session is cleared by the profile-fetch 401. Its `disabled`+`hint` path is covered only by build and lint. **Worth a click-through.**
...
- **Six undocumented env vars** missing from `backend/.env.example`: `JWT_SECRET`, `ADMIN_EMAIL`, `ADMIN_PASSWORD_HASH`, `CLOUDINARY_*`, `RESEND_API_KEY`, `GOOGLE_CLIENT_ID`. A fresh deploy from the template comes up with **no admin login and no image uploads**.
