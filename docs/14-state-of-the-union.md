...
- **⚠️ One staging CLS run at 0.0702, over the 0.05 gate (2026-07-19, post-Phase-5).** Pre-Phase-5 staging was 0.015 on all five runs; post-merge it is 0.0156 on four (unchanged) and 0.0702 on one. Median passes. Not a systematic regression from the parallax transform, which is out of flow and excluded from layout-shift scoring — the outlier landed on the slowest run, which fits the known API-latency-dependent hero ticker skeleton swap. **Hypothesis, not a finding.** Re-measure on the next staging capture; if it recurs, reserve space for the ticker cards. `docs/13 §5g`.
- **`npm run lh:cinematic` is unreliable against staging.** Lighthouse stops recording network before the trace ends, and staging is fast enough that the scene mounts after that cutoff — so the script refuses the capture (1/5 runs). It fails safe, and the layer is verified working on staging in a real browser, but **the desktop cost numbers in §5e/§5f rest on the local A/B and there is no validated staging measurement of the canvas.** Fix by asserting on the trace or a DOM probe instead of the `network-requests` audit. `docs/13 §5g`.
